...
public class Board {
	...
	/*
	 * Returns the set of squares visited by the knight, keeping track
	 * of the move number that the square was visited, starting with 1.
	 * Because Warnsdorff's heuristic is based on moving to the square
	 * with the least possible moves, it makes sense to start in a corner.
	 */
	public Square[][] findTour()
	{
		Square current = squares[0][0];			//Start in the top left.
		for (int i = 0; i < files * ranks; i++)	//Loop until each square is visited.
		{
	...
	/*
	 * Based on the current position on the board, this method finds the next move.
	 * Following Warnsdorff's heuristic, the next move is the square with the lowest
	 * number of possible moves. If there are multiple square that qualify, the first
	 * one found is visited.
	 */
	...
	/*
	 * Given a position on the chessboard, return a list of moves
	 * that the knight can make. Moves are valid if they are within
	 * the domain of the chess board, and have not already been
	 * visited.
	 */
	...
	/*
	 * Checks if the next move is within the domain of the chessboard. Both x and y coordinates
	 * must fall inclusively between 0 and files - 1 and ranks - 1, respectively.
	 */
	...
	/* Returns the final chessboard labeled with each move number.
	 * Squares not visited are marked in red.
	 */
	...
	/*
	 * Create a stackpane to model the chessboard row by row given its knight's tour.
	 * The chessboard is blue and white, marking unvisitied squares in red.
	 */
	private StackPane draw(Square[][] squares)
	{
		...
		{	
			...
			{				
				...
				//If the square was not visited, color it red, otherwise, color it in an
	...
	/*
	 * Data structure to represent each square on a chessboard. The x and y coordinates begin
	 * at the top left (0, 0) and end at the bottom right (files - 1, ranks - 1).
	 */
